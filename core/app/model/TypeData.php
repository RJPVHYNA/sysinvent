...
class TypeData {
	...
	public static function delById($id){
		$sql = "delete from ".self::$tablename." where id_tipo=$id";
		Executor::doit($sql);
	}
	public function del(){
		$sql = "delete from ".self::$tablename." where id_tipo=$this->id";
		Executor::doit($sql);
	}

// partiendo de que ya tenemos creado un objecto KindData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set name=\"$this->name\" where id_tipo=$this->id";
		Executor::doit($sql);
	}
